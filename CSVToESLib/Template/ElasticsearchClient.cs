...
namespace CSVToESLib.Template
{
    internal class ElasticsearchClient
    {
        ...
        {
            ParallelQuery<string> test = results.Where(result => result.IsValid).Select(result => { result.Result.Version = version; return new Index(result.Result).ToString(); });
